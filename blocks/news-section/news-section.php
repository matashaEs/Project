    ...
    <div class="container news-section__container">
        ...
        <div class="row news-section__row news-section__row-news">
			<?php if ( ! empty( $selected_news ) ) : ?>
				<?php $posts = apply_filters( 'cai_get_selected_news', $selected_news ); ?>
			<?php elseif ( ! empty( $categories ) ) : ?>
				<?php $category = get_term( $categories )->slug; ?>
				<?php $posts = apply_filters( 'cai_get_filtered_news', $category ); ?>
			<?php else : ?>
				<?php $posts = apply_filters( 'cai_get_filtered_news', null ); ?>
			<?php endif; ?>
			<?php foreach ( $posts as $post ) { ?>
                <a href="<?= esc_url( $post['url'] ); ?>"
                   class="news-section__item modular__item--mobile">
                    <div class="news-section__item-top-content">
                        <div class="p news-section__item-date"><?= esc_html( $post['date'] ); ?></div>
                    ...
                    <div class="h4 news-section__item-title"><?= esc_html( $post['name'] ); ?></div>
                    <div class="p news-section__item-read-more">
						<?= esc_html( __( 'Read More', 'nuplo' ) ) ?>
                    </div>
                </a>
				<?php if ( ++ $i == $how_much_news ) {
					break;
				} ?>
			<?php } ?>
        </div>
